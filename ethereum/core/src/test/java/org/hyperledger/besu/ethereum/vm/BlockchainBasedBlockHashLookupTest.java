...
class BlockchainBasedBlockHashLookupTest {
  ...
  @Test
  void getAccessedBlockHashesContainsParentByDefault() {
    final long parentNumber = CURRENT_BLOCK_NUMBER - 1L;
    assertThat(lookup.getAccessedBlockHashes())
        .containsEntry(parentNumber, headers[(int) parentNumber].getHash())
        .hasSize(1);
  }

  @Test
  void getAccessedBlockHashesRecordsEveryBlockWalkedByBlockhashLookups() {
    ...
    final Map<Long, Hash> resolved = lookup.getAccessedBlockHashes();
  ...
  @Test
  void getAccessedBlockHashesReturnsUnmodifiableView() {
    Assertions.assertThatThrownBy(() -> lookup.getAccessedBlockHashes().put(1L, Hash.ZERO))
