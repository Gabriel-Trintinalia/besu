...
public class BlockStateCallResult extends BlockResult {
  ...
  public static BlockStateCallResult create(
    ...
    // The receipts cannot be used directly as they were filtered out during the simulation
    // to exclude logs generated by ETH transfers. Therefore, we need to generate the logs
    // from the transaction simulation results.
    List<LogWithMetadata> logs = generateLogs(simulationResult, block);
  ...
  /**
   * Generates a list of {@link LogWithMetadata} objects from the logs produced by transaction
   * processing.
   *
   * @param simulationResult the result of simulating a block
   * @param block the block containing the transactions
   * @return a list of {@link LogWithMetadata} objects representing the logs produced by transaction
   *     processing
   */
