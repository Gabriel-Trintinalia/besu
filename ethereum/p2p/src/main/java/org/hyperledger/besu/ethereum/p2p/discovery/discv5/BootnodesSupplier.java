...
/**
 * Supplies default discv5 bootnodes.
 *
 * <p>Currently returns a static set of well-known public bootnodes. This mirrors the existing
 * discv4 behavior and acts as a stopgap until bootnodes can be fully configured via {@link
 * DiscoveryConfiguration}.
 */
public final class BootnodesSupplier {

  private BootnodesSupplier() {
    // utility class
  }

  static List<NodeRecord> get(final DiscoveryConfiguration config) {
    // config intentionally unused for now (future: user-configured bootnodes)
    return BOOTNODES_V5.stream().map(NodeRecordFactory.DEFAULT::fromEnr).toList();
  }

  private static final List<String> BOOTNODES_V5 =
      List.of( // Teku team's bootnode
